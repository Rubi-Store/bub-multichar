...
local function giveStarterItems(source)
  ...
  while not exports['qs-inventory']:GetInventory(source) do
		Wait(100)
	end
	for i = 1, #starterItems do
		local item = starterItems[i]
		if item.metadata and type(item.metadata) == 'function' then
      exports['qs-inventory']:AddItem(source, item.name, item.amount, nil, item.metadata(source))
		else
      exports['qs-inventory']:AddItem(source, item.name, item.amount, nil, item.metadata)
		end
	end
end
